using System;
using Microsoft.EntityFrameworkCore.Migrations;

#nullable disable

#pragma warning disable CA1814 // Prefer jagged arrays over multidimensional

namespace be.Migrations
{
    /// <inheritdoc />
    public partial class updateseed : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DeleteData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P3-Img2");

            migrationBuilder.DeleteData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P3-Img3");

            migrationBuilder.DeleteData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P3-Img4");

            migrationBuilder.AddColumn<string>(
                name: "ImgUrl",
                table: "Brands",
                type: "text",
                nullable: true);

            migrationBuilder.UpdateData(
                table: "Brands",
                keyColumn: "Id",
                keyValue: "1",
                column: "ImgUrl",
                value: "data:image/png;base64,");

            migrationBuilder.UpdateData(
                table: "Brands",
                keyColumn: "Id",
                keyValue: "2",
                column: "ImgUrl",
                value: "data:image/png;base64,");

            migrationBuilder.UpdateData(
                table: "Brands",
                keyColumn: "Id",
                keyValue: "3",
                column: "ImgUrl",
                value: "data:image/png;base64,");

            migrationBuilder.UpdateData(
                table: "Brands",
                keyColumn: "Id",
                keyValue: "4",
                column: "ImgUrl",
                value: "data:image/png;base64,");

            migrationBuilder.UpdateData(
                table: "Brands",
                keyColumn: "Id",
                keyValue: "5",
                column: "ImgUrl",
                value: "data:image/png;base64,");

            migrationBuilder.UpdateData(
                table: "Brands",
                keyColumn: "Id",
                keyValue: "6",
                column: "ImgUrl",
                value: "data:image/png;base64,");

            migrationBuilder.InsertData(
                table: "Categories",
                columns: new[] { "Id", "IsAvailable", "Name", "ParentCategoryId" },
                values: new object[] { "7", true, "Other", null });

            migrationBuilder.UpdateData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P1-Img1",
                column: "CreatedAt",
                value: new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7495));

            migrationBuilder.UpdateData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P1-Img2",
                column: "CreatedAt",
                value: new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7496));

            migrationBuilder.UpdateData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P1-Img3",
                column: "CreatedAt",
                value: new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7497));

            migrationBuilder.UpdateData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P1-Img4",
                column: "CreatedAt",
                value: new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7498));

            migrationBuilder.UpdateData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P2-Img1",
                column: "CreatedAt",
                value: new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7499));

            migrationBuilder.UpdateData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P2-Img2",
                column: "CreatedAt",
                value: new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7500));

            migrationBuilder.UpdateData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P2-Img3",
                column: "CreatedAt",
                value: new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7502));

            migrationBuilder.UpdateData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P2-Img4",
                column: "CreatedAt",
                value: new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7503));

            migrationBuilder.UpdateData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P3-Img1",
                columns: new[] { "CreatedAt", "ImageUrl" },
                values: new object[] { new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7504), "https://resource.logitech.com/w_386,ar_1.0,c_limit,f_auto,q_auto,dpr_2.0/d_transparent.gif/content/dam/logitech/en/products/mice/pop-wireless-mouse/gallery/pop-mouse-gallery-off-white-1.png?v=1" });

            migrationBuilder.UpdateData(
                table: "Products",
                keyColumn: "Id",
                keyValue: "1",
                columns: new[] { "CreatedAt", "UpdatedAt" },
                values: new object[] { new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7369), new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7371) });

            migrationBuilder.UpdateData(
                table: "Products",
                keyColumn: "Id",
                keyValue: "2",
                columns: new[] { "CreatedAt", "UpdatedAt" },
                values: new object[] { new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7373), new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7374) });

            migrationBuilder.UpdateData(
                table: "Products",
                keyColumn: "Id",
                keyValue: "3",
                columns: new[] { "BrandId", "CategoryId", "CreatedAt", "Description", "Name", "Price", "Sku", "StockQuantity", "UpdatedAt" },
                values: new object[] { "1", "3", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7378), "Wireless POP Mouse", "POP Mouse", 849000m, "POP123", 50m, new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7380) });

            migrationBuilder.InsertData(
                table: "Products",
                columns: new[] { "Id", "BrandId", "CartQuantity", "CategoryId", "CreatedAt", "Description", "IsAvailable", "Name", "Price", "Sku", "StockQuantity", "UpdatedAt" },
                values: new object[,]
                {
                    { "10", "1", null, "3", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7397), "Ergonomic vertical mouse", true, "LIFT", 2500000m, "LIFT123", 80m, new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7398) },
                    { "11", "1", null, "3", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7400), "Sleek and silent mouse", true, "Pebble Mouse 2 M350s", 2139990m, "PEBBLEM2", 75m, new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7401) },
                    { "12", "1", null, "3", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7403), "Wireless mouse for everyday use", true, "Signature M650", 1700000m, "SIGM650", 65m, new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7404) },
                    { "13", "1", null, "3", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7406), "Multi-device wireless mouse", true, "M720 Triathlon", 1599000m, "M720TRI", 85m, new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7407) },
                    { "14", "1", null, "3", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7408), "Silent wireless mouse", true, "M331 SILENT PLUS", 2200000m, "M331SP", 90m, new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7409) },
                    { "4", "1", null, "3", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7381), "Ergonomic Wireless Trackball", true, "ERGO M575S", 1800000m, "ERGOM575S", 40m, new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7382) },
                    { "5", "1", null, "3", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7384), "Precision trackball mouse", true, "MX Ergo S", 1329000m, "MXERGO", 35m, new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7385) },
                    { "6", "1", null, "3", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7387), "Compact wireless mouse for Mac", true, "MX Anywhere 3S for Mac", 1500000m, "MXA3SMAC", 60m, new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7388) },
                    { "7", "1", null, "3", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7389), "High-performance wireless mouse", true, "MX Master 3S", 3299000m, "MXMASTER3S", 45m, new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7390) },
                    { "8", "1", null, "3", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7392), "High-precision mouse for Mac", true, "MX Master 3S For Mac", 1200000m, "MXM3SMAC", 50m, new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7393) },
                    { "9", "1", null, "3", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7395), "Portable wireless mouse", true, "MX Anywhere 3S", 3299000m, "MXA3S", 70m, new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7396) }
                });

            migrationBuilder.UpdateData(
                table: "Ratings",
                keyColumn: "Id",
                keyValue: "R1",
                columns: new[] { "CreatedAt", "UpdatedAt" },
                values: new object[] { new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7441), new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7442) });

            migrationBuilder.UpdateData(
                table: "Ratings",
                keyColumn: "Id",
                keyValue: "R2",
                columns: new[] { "CreatedAt", "UpdatedAt" },
                values: new object[] { new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7443), new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7444) });

            migrationBuilder.UpdateData(
                table: "Ratings",
                keyColumn: "Id",
                keyValue: "R3",
                columns: new[] { "CreatedAt", "UpdatedAt" },
                values: new object[] { new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7445), new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7446) });

            migrationBuilder.UpdateData(
                table: "Ratings",
                keyColumn: "Id",
                keyValue: "R4",
                columns: new[] { "CreatedAt", "RatingValue", "Review", "UpdatedAt" },
                values: new object[] { new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7447), 5, "Stylish and compact!", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7447) });

            migrationBuilder.InsertData(
                table: "ProductImage",
                columns: new[] { "Id", "CreatedAt", "ImageUrl", "ProductId" },
                values: new object[,]
                {
                    { "P10-Img1", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7568), "https://resource.logitech.com/w_386,ar_1.0,c_limit,f_auto,q_auto,dpr_2.0/d_transparent.gif/content/dam/logitech/en/products/mice/pop-wireless-mouse/gallery/wave-2/pop-mouse-gallery-cosmo-1.png?v=1", "10" },
                    { "P11-Img1", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7569), "https://resource.logitech.com/w_386,ar_1.0,c_limit,f_auto,q_auto,dpr_2.0/d_transparent.gif/content/dam/logitech/en/products/mice/pop-wireless-mouse/gallery/pop-mouse-gallery-daydream-1.png?v=1", "11" },
                    { "P12-Img1", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7570), "https://resource.logitech.com/w_386,ar_1.0,c_limit,f_auto,q_auto,dpr_2.0/d_transparent.gif/content/dam/logitech/en/products/mice/m575s-ergo-wireless-trackball/gallery/ergo-m575s-m575sp-graphite-blue-ball-gallery-1.png?v=1", "12" },
                    { "P13-Img1", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7571), "https://resource.logitech.com/w_386,ar_1.0,c_limit,f_auto,q_auto,dpr_2.0/d_transparent.gif/content/dam/logitech/en/products/mice/mx-ergo-s-wireless-trackball-mouse/gallery/mx-ergo-s-graphite-gallery-1.png?v=1", "13" },
                    { "P14-Img1", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7572), "https://resource.logitech.com/w_386,ar_1.0,c_limit,f_auto,q_auto,dpr_2.0/d_transparent.gif/content/dam/logitech/en/products/mice/mx-anywhere-3s-for-mac/buy/gallery/mx-anywhere-3s-mac-space-grey-gallery-01.png?v=1", "14" },
                    { "P4-Img1", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7505), "https://resource.logitech.com/w_386,ar_1.0,c_limit,f_auto,q_auto,dpr_2.0/d_transparent.gif/content/dam/logitech/en/products/mice/pop-wireless-mouse/gallery/pop-mouse-gallery-rose-1.png?v=1", "4" },
                    { "P5-Img1", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7562), "https://resource.logitech.com/w_386,ar_1.0,c_limit,f_auto,q_auto,dpr_2.0/d_transparent.gif/content/dam/logitech/en/products/mice/pop-wireless-mouse/gallery/pop-mouse-gallery-graphite-1.png?v=1", "5" },
                    { "P6-Img1", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7564), "https://resource.logitech.com/w_386,ar_1.0,c_limit,f_auto,q_auto,dpr_2.0/d_transparent.gif/content/dam/logitech/en/products/mice/pop-wireless-mouse/gallery/wave-2/pop-mouse-gallery-mist-1.png?v=1", "6" },
                    { "P7-Img1", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7565), "https://resource.logitech.com/w_386,ar_1.0,c_limit,f_auto,q_auto,dpr_2.0/d_transparent.gif/content/dam/logitech/en/products/mice/pop-wireless-mouse/gallery/pop-mouse-gallery-blast-1.png?v=1", "7" },
                    { "P8-Img1", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7566), "https://resource.logitech.com/w_386,ar_1.0,c_limit,f_auto,q_auto,dpr_2.0/d_transparent.gif/content/dam/logitech/en/products/mice/pop-wireless-mouse/gallery/pop-mouse-gallery-heartbreak-1.png?v=1", "8" },
                    { "P9-Img1", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7567), "https://resource.logitech.com/w_386,ar_1.0,c_limit,f_auto,q_auto,dpr_2.0/d_transparent.gif/content/dam/logitech/en/products/mice/pop-wireless-mouse/gallery/2024-update/pop-mouse-gallery-lilac-1.png?v=1", "9" }
                });

            migrationBuilder.InsertData(
                table: "Ratings",
                columns: new[] { "Id", "CreatedAt", "ProductId", "RatingValue", "Review", "UpdatedAt", "UserId" },
                values: new object[,]
                {
                    { "R10", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7457), "10", 5, "Ergonomic design, very comfortable.", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7457), "U10" },
                    { "R11", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7459), "11", 5, "Silent and sleek mouse.", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7459), "U11" },
                    { "R12", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7460), "12", 5, "Perfect for work and everyday use.", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7461), "U12" },
                    { "R13", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7462), "13", 4, "Good value for multi-device users.", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7463), "U13" },
                    { "R14", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7464), "14", 4, "Silent clicks make it great for quiet spaces.", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7464), "U14" },
                    { "R5", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7448), "4", 4, "Great ergonomic design.", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7449), "U5" },
                    { "R6", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7450), "5", 4, "Smooth and precise!", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7451), "U6" },
                    { "R7", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7452), "7", 4, "Amazing precision and feel.", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7452), "U7" },
                    { "R8", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7454), "8", 5, "Great for professional use.", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7454), "U8" },
                    { "R9", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7455), "9", 4, "Portable and reliable.", new DateTime(2024, 12, 22, 13, 29, 28, 370, DateTimeKind.Utc).AddTicks(7456), "U9" }
                });
        }

        /// <inheritdoc />
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DeleteData(
                table: "Categories",
                keyColumn: "Id",
                keyValue: "7");

            migrationBuilder.DeleteData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P10-Img1");

            migrationBuilder.DeleteData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P11-Img1");

            migrationBuilder.DeleteData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P12-Img1");

            migrationBuilder.DeleteData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P13-Img1");

            migrationBuilder.DeleteData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P14-Img1");

            migrationBuilder.DeleteData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P4-Img1");

            migrationBuilder.DeleteData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P5-Img1");

            migrationBuilder.DeleteData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P6-Img1");

            migrationBuilder.DeleteData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P7-Img1");

            migrationBuilder.DeleteData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P8-Img1");

            migrationBuilder.DeleteData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P9-Img1");

            migrationBuilder.DeleteData(
                table: "Ratings",
                keyColumn: "Id",
                keyValue: "R10");

            migrationBuilder.DeleteData(
                table: "Ratings",
                keyColumn: "Id",
                keyValue: "R11");

            migrationBuilder.DeleteData(
                table: "Ratings",
                keyColumn: "Id",
                keyValue: "R12");

            migrationBuilder.DeleteData(
                table: "Ratings",
                keyColumn: "Id",
                keyValue: "R13");

            migrationBuilder.DeleteData(
                table: "Ratings",
                keyColumn: "Id",
                keyValue: "R14");

            migrationBuilder.DeleteData(
                table: "Ratings",
                keyColumn: "Id",
                keyValue: "R5");

            migrationBuilder.DeleteData(
                table: "Ratings",
                keyColumn: "Id",
                keyValue: "R6");

            migrationBuilder.DeleteData(
                table: "Ratings",
                keyColumn: "Id",
                keyValue: "R7");

            migrationBuilder.DeleteData(
                table: "Ratings",
                keyColumn: "Id",
                keyValue: "R8");

            migrationBuilder.DeleteData(
                table: "Ratings",
                keyColumn: "Id",
                keyValue: "R9");

            migrationBuilder.DeleteData(
                table: "Products",
                keyColumn: "Id",
                keyValue: "10");

            migrationBuilder.DeleteData(
                table: "Products",
                keyColumn: "Id",
                keyValue: "11");

            migrationBuilder.DeleteData(
                table: "Products",
                keyColumn: "Id",
                keyValue: "12");

            migrationBuilder.DeleteData(
                table: "Products",
                keyColumn: "Id",
                keyValue: "13");

            migrationBuilder.DeleteData(
                table: "Products",
                keyColumn: "Id",
                keyValue: "14");

            migrationBuilder.DeleteData(
                table: "Products",
                keyColumn: "Id",
                keyValue: "4");

            migrationBuilder.DeleteData(
                table: "Products",
                keyColumn: "Id",
                keyValue: "5");

            migrationBuilder.DeleteData(
                table: "Products",
                keyColumn: "Id",
                keyValue: "6");

            migrationBuilder.DeleteData(
                table: "Products",
                keyColumn: "Id",
                keyValue: "7");

            migrationBuilder.DeleteData(
                table: "Products",
                keyColumn: "Id",
                keyValue: "8");

            migrationBuilder.DeleteData(
                table: "Products",
                keyColumn: "Id",
                keyValue: "9");

            migrationBuilder.DropColumn(
                name: "ImgUrl",
                table: "Brands");

            migrationBuilder.UpdateData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P1-Img1",
                column: "CreatedAt",
                value: new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6560));

            migrationBuilder.UpdateData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P1-Img2",
                column: "CreatedAt",
                value: new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6562));

            migrationBuilder.UpdateData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P1-Img3",
                column: "CreatedAt",
                value: new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6563));

            migrationBuilder.UpdateData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P1-Img4",
                column: "CreatedAt",
                value: new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6564));

            migrationBuilder.UpdateData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P2-Img1",
                column: "CreatedAt",
                value: new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6565));

            migrationBuilder.UpdateData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P2-Img2",
                column: "CreatedAt",
                value: new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6566));

            migrationBuilder.UpdateData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P2-Img3",
                column: "CreatedAt",
                value: new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6567));

            migrationBuilder.UpdateData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P2-Img4",
                column: "CreatedAt",
                value: new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6568));

            migrationBuilder.UpdateData(
                table: "ProductImage",
                keyColumn: "Id",
                keyValue: "P3-Img1",
                columns: new[] { "CreatedAt", "ImageUrl" },
                values: new object[] { new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6569), "https://placehold.co/400x400" });

            migrationBuilder.InsertData(
                table: "ProductImage",
                columns: new[] { "Id", "CreatedAt", "ImageUrl", "ProductId" },
                values: new object[,]
                {
                    { "P3-Img2", new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6570), "https://placehold.co/400x400/gray", "3" },
                    { "P3-Img3", new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6571), "https://placehold.co/400x400/black", "3" },
                    { "P3-Img4", new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6572), "https://placehold.co/400x400/blue", "3" }
                });

            migrationBuilder.UpdateData(
                table: "Products",
                keyColumn: "Id",
                keyValue: "1",
                columns: new[] { "CreatedAt", "UpdatedAt" },
                values: new object[] { new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6478), new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6482) });

            migrationBuilder.UpdateData(
                table: "Products",
                keyColumn: "Id",
                keyValue: "2",
                columns: new[] { "CreatedAt", "UpdatedAt" },
                values: new object[] { new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6484), new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6486) });

            migrationBuilder.UpdateData(
                table: "Products",
                keyColumn: "Id",
                keyValue: "3",
                columns: new[] { "BrandId", "CategoryId", "CreatedAt", "Description", "Name", "Price", "Sku", "StockQuantity", "UpdatedAt" },
                values: new object[] { "2", "2", new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6487), "Bestselling novel book", "Novel Book", 100000m, "NBK789", 300m, new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6489) });

            migrationBuilder.UpdateData(
                table: "Ratings",
                keyColumn: "Id",
                keyValue: "R1",
                columns: new[] { "CreatedAt", "UpdatedAt" },
                values: new object[] { new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6519), new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6519) });

            migrationBuilder.UpdateData(
                table: "Ratings",
                keyColumn: "Id",
                keyValue: "R2",
                columns: new[] { "CreatedAt", "UpdatedAt" },
                values: new object[] { new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6521), new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6521) });

            migrationBuilder.UpdateData(
                table: "Ratings",
                keyColumn: "Id",
                keyValue: "R3",
                columns: new[] { "CreatedAt", "UpdatedAt" },
                values: new object[] { new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6523), new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6523) });

            migrationBuilder.UpdateData(
                table: "Ratings",
                keyColumn: "Id",
                keyValue: "R4",
                columns: new[] { "CreatedAt", "RatingValue", "Review", "UpdatedAt" },
                values: new object[] { new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6525), 4, "Engaging and well-written.", new DateTime(2024, 12, 22, 10, 26, 3, 89, DateTimeKind.Utc).AddTicks(6525) });
        }
    }
}
